...
void MainWindow::vehicleCallback(const remote_truck::Info &msg)
{
  ...
  //ui->delay_label->setText(QString::number(delay));
...
//  //ROS_INFO("paint received!");
//  //QPainter painter(ui->line_label);
...
//  if(line_type == 1){
//    for(int i=0;i<30;i++){
//      pointf[i].setX(1720 + (int)3*x_array[i]);
//      pointf[i].setY(618 - (int)3*y_array[i]);
//    }
//  }else{
//    for(int i=0;i<30;i++){
//      pointf[i].setX(1720 - (int)3*x_array[i]);
...
//  QLineF lineLeft(1008 + 600, 618, 1051 + 600, 389);//(x1,y1)and(x2,y2)

//  painter.drawLine(lineLeft);

//  QLineF lineRight(1215 + 600, 618, 1172 + 600, 389);
